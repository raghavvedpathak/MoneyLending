...
import '../../../data/models/item_rate_entity.dart';

/// DAO for ItemRateEntity.
///
/// Mandated by Data Spec §4.1, §4.4, §4.5 & [FIX-ARCH-ITEMRATE-1]:
/// - Unique index on (itemCategory, effectiveDate).
/// - Never use @Insert(onConflict = OnConflictStrategy.REPLACE) / InsertMode.insertOrReplace —
///   REPLACE deletes and reinserts the row, reassigning the id UUID.
...
class ItemRateDao {
  ...
  /// effectiveDate is on or before [dateOnly] (date-only); null when there is none.
  /// Used to auto-fill the item rate of a backdated record.
  ...
  Future<ItemRateEntity?> getRateAsOf(String category, String dateOnly) async {
    final maps = await _db.query(
      'item_rates',
      where: 'itemCategory = ? AND effectiveDate <= ? AND ratePerUnit > 0',
      whereArgs: [category, dateOnly],
      orderBy: 'effectiveDate DESC, updatedAt DESC',
      limit: 1,
    );
    if (maps.isEmpty) return null;
